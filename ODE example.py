...
def ode_system_herm(y, t, P):
    ...
    for l in range(len(dydt)):
        dydt[l] = -(sum(sum(inner3_herm(i, j, l)*ki_herm[i]*y[j] for j in range(P+1))
                        for i in range(P+1)))/inner2_herm(l)
    return dydt
...
for i in range(5):
    for j in range(5):
        error_r_D[i, j] = (
            np.mean((np.abs(pfprior_dens(qsample) - pfprior_dens_n(j+1, qsample)))**(i+1)))**(1/(i+1))
...
# Observation is $\pi_{\mathcal{D}}\sim N(1,0.1)$
...
for i in range(5):
    error_update[i] = (
        np.mean((np.abs(pdf_update(0, lamsample) - pdf_update(i+1, lamsample)))**2))**(1/2)
